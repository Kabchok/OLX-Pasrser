...
with sync_playwright() as p:
    ...
    page.goto(
        "https://www.olx.ua/uk/nedvizhimost/kvartiry/prodazha-kvartir/kropivnitskiy/?currency=USD&search%5Bfilter_enum_number_of_rooms_string%5D%5B0%5D=trehkomnatnye&search%5Bfilter_float_price%3Afrom%5D=30000&search%5Bfilter_float_price%3Ato%5D=75000&search%5Bfilter_float_total_area%3Afrom%5D=50&search%5Bfilter_float_total_area%3Ato%5D=75&search%5Border%5D=created_at%3Adesc",
        timeout=60000)
...
# Подключаемся к БД
conn = sqlite3.connect("data/flats3.db")
cursor = conn.cursor()

# Создаём таблицу
cursor.execute("""
               CREATE TABLE IF NOT EXISTS flats3
               (
                   id
                   INTEGER
                   PRIMARY
...
cursor.execute("SELECT MAX(parsed_at) FROM flats3")
...
sr_price_60_70 = round(sum(filtered_prices) / len(filtered_prices))
print("Средняя стоимость квартир от 60 до 70: ", sr_price_60_70)
...
# Создаём таблицу
cursor2.execute("""
                CREATE TABLE IF NOT EXISTS pokazateli3
                (
                    id
                    INTEGER
                    PRIMARY
                ...
                """)
cursor2.execute("""
                INSERT INTO pokazateli3 (avg_price, avg_area, avg_price_per_m2, avg_60_70, date)
                VALUES (?, ?, ?, ?, ?)
                """, (average_price, average_area, price_1m, sr_price_60_70, now))
