...
with sync_playwright() as p:
    ...
    page.goto(
        "https://www.olx.ua/uk/nedvizhimost/kvartiry/prodazha-kvartir/kropivnitskiy/?currency=USD&search%5Bfilter_enum_number_of_rooms_string%5D%5B0%5D=dvuhkomnatnye&search%5Bfilter_float_price%3Afrom%5D=22000&search%5Bfilter_float_price%3Ato%5D=50000&search%5Bfilter_float_total_area%3Afrom%5D=40&search%5Bfilter_float_total_area%3Ato%5D=60&search%5Border%5D=created_at%3Adesc",
        timeout=60000)
...
# Подключаемся к БД
conn = sqlite3.connect("data/flats2.db")
cursor = conn.cursor()
...
cursor.execute("SELECT MAX(parsed_at) FROM flats2")
...
sr_price_46_55 = round(sum(filtered_prices) / len(filtered_prices))
print("Средняя стоимость квартир от 46 до 55: ", sr_price_46_55)
...
# Создаём таблицу
cursor2.execute("""
                CREATE TABLE IF NOT EXISTS pokazateli2
                (
                    id
                    INTEGER
                    PRIMARY
                ...
                """)
cursor2.execute("""
                INSERT INTO pokazateli2 (avg_price, avg_area, avg_price_per_m2, avg_46_55, date)
                VALUES (?, ?, ?, ?, ?)
                """, (average_price, average_area, price_1m, sr_price_46_55, now))
